...
public class ParticipantTest extends Base {
	...
	public void participantValidationTest() throws Exception {
		...
		Reporter.log("chat session is sucessfully displayed in  Participant Test ..>>>>>>>>>>>", true);

		try {
			Thread.sleep(4000);

			Utility.isElementPresnt(driver, "//input[@id='input']", 10).sendKeys("@participant");

			Utility.isElementPresnt(driver, "//span[text()='Patient can submit chats']", 10).click();

			boolean b = close.getParticipantFull().getText().contains("Patient");
		...
		Reporter.log(
				"ParticipantTest TestCase is sucessfully done ################################################## TEST IS PASS",
				true);
