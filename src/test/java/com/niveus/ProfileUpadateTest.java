...
public class ProfileUpadateTest extends Base {
	...
	@Test(dependsOnMethods = "updateProfileInformation")
	public void profileChangeTest() throws Exception {

		try {
			Reporter.log("profile update  Test Started  ###########################", true);

			Utility.isElementPresnt(driver, "//a[@href='/profile']", 10).click();

			Utility.isElementPresntOnId(driver, "first_name", 10);

			propage.updateProfile("T", "a", "Female");

			int count = 0;
			while (count == 0) {
				propage.getUpdatebtn().submit();
				count++;
			}

			wait.until(ExpectedConditions.visibilityOf(propage.getUpdateinformtion()));

			AssertJUnit.assertEquals(propage.getUpdateinformtion().getText(), "information successfully Updated");

			Reporter.log("profile information is sucessfully updated");
			Reporter.log("profile UpdateTest TestCase is sucessfully done ############################# TEST IS PASS",
					true);
		} catch (Exception e) {
		}
	}
